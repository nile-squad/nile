...
```
[Client Request] → [Authentication] → [Authorization (onActionHandler)] → [Payload Validation] → [Before Hooks] → [Action Handler] → [After Hooks]
```

**Key Points:**
- Authentication happens FIRST (identity verification)
- Authorization happens SECOND (permission checks via onActionHandler)
- onActionHandler receives: `(context: NileContext, action: Action, payload: unknown, stage?: 'before' | 'after')`
...
const actionHook: ActionHookHandler = (context, action, payload, stage) => {
  // context: NileContext with user, session, request
  // action: Full action definition
  // payload: Request payload
  // stage: 'before' | 'after' | undefined
...
1. **`context`** - NileContext with user, session, request data
2. **`action`** - Full action definition (Action object)
3. **`payload`** - Request payload
4. **`stage`** - Optional stage indicator ('before' | 'after')
...
export const accessControlHook: ActionHookHandler = (context, action, _payload, stage) => {
  const { user } = context;
