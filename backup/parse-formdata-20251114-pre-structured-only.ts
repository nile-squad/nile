 ...
 * Handles flat and structured modes with security validations
 */
...
export function validateFiles(
  ...
  if (!filenameCheck.status) return filenameCheck;

  const zeroByteCheck = validateZeroByteFiles(files);
  if (!zeroByteCheck.status) return zeroByteCheck;

  const countCheck = validateFileCount(files, maxFiles);
  if (!countCheck.status) return countCheck;

  const sizeCheck = validateFileSize(files, maxFileSize);
  if (!sizeCheck.status) return sizeCheck;

  const totalSizeCheck = validateTotalSize(files, maxTotalSize);
  if (!totalSizeCheck.status) return totalSizeCheck;
  ...
  if (!allowlistCheck.status) return allowlistCheck;

  return { status: true };
}

/**
 * Parse FormData into flat payload (backward compatible mode)
 * Last value wins for duplicate keys
 * @param formData - The FormData instance to parse
 * @returns Flat payload object
 */
export function parseFormDataFlat(formData: FormData): FlatPayload {
 ...
 * Aggregates duplicate keys into arrays
 * @param formData - The FormData instance to parse
 * @returns Structured payload with separate fields and files
 */
export function parseFormDataStructured(formData: FormData): StructuredPayload {
 ...
 * @param formData - The FormData instance to parse
 * @param mode - Parsing mode ('flat' or 'structured')
 * @returns Parsed payload in the requested format
 */
export function parseFormData(
  formData: FormData,
  mode: ParseMode = 'flat'
): FlatPayload | StructuredPayload {
  if (mode === 'structured') {
    return parseFormDataStructured(formData);
  }
  return parseFormDataFlat(formData);
}
