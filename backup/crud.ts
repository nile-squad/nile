...
export function createCrudOperations<TTable extends Table>(
  ...
  function processJsonUpdates(
    ...
    for (const key of Object.keys(changes)) {
      ...
      // Check if this is a JSON column that needs deep merging
      if (
        ...
        // Get the existing full object from the database
        const existingData = oldData[key as keyof SelectType];

        // value from getChanges is the partial change (e.g., { preferences: { theme: "dark" } })
        // We need to merge this partial change into the full existing object
        if (
          existingData &&
  ...
  /**
   * Helper function to build select object for field projection
   */
  ...
  const findFirst = async (
    ...
    try {
      let whereClause: ReturnType<typeof buildWhereClause>;
      whereClause = buildWhereClause(table, filters, dialect);
