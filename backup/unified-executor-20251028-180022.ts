...
export async function executeUnified(
  ...
  const lookupDuration =
    lookupReport.stages[lookupReport.stages.length - 1]?.duration;
  logDiagnostic(
    serverConfig.diagnostics,
  ...
  try {
    ...
    const authDuration =
      authReport.stages[authReport.stages.length - 1]?.duration;
    ...
    const beforeHookDuration =
      beforeHookReport.stages[beforeHookReport.stages.length - 1]?.duration;
    logDiagnostic(
      serverConfig.diagnostics,
      'global-before-hook',
      serviceName,
    ...
    const validationDuration =
      validationReport.stages[validationReport.stages.length - 1]?.duration;
    logDiagnostic(
      serverConfig.diagnostics,
      'payload-validation',
      serviceName,
    ...
    if (action.hooks?.before && action.hooks.before.length > 0) {
      ...
      const actionsMap = new Map(service.actions.map((act) => [act.name, act]));
      ...
      for (const hookDef of action.hooks.before) {
        ...
        const hookDuration =
          hookReport.stages[hookReport.stages.length - 1]?.duration;
    ...
    const handlerDuration =
      handlerReport.stages[handlerReport.stages.length - 1]?.duration;
    logDiagnostic(
      serverConfig.diagnostics,
      'main-handler',
      serviceName,
    ...
    const afterHookDuration =
      afterHookReport.stages[afterHookReport.stages.length - 1]?.duration;
    logDiagnostic(
      serverConfig.diagnostics,
      'global-after-hook',
      serviceName,
