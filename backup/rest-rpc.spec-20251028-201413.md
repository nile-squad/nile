...
**Action Hooks** provide global execution logic for cross-cutting concerns like authorization, rate limiting, and audit logging. Unlike action-level hooks which run within a specific action's workflow, global action hooks run before and/or after **every** action across **all** services.

### Hook Execution Pipeline

```
[Request] → [Authentication] → [onBeforeActionHandler] → [Payload Validation] → [Action Handler] → [onAfterActionHandler] → [Response]
```

Action Hooks execute:
- **onBeforeActionHandler**: After authentication but before payload validation (authorization)
- **onAfterActionHandler**: After action execution (auditing, logging)

### Configuration

Global Action Hooks are configured in the server configuration:
...
const globalAccessControl: OnBeforeActionHandler = ({ nileContext, action, payload }) => {
  ...
  if (!allowedActions.includes('*') && !allowedActions.includes(actionName)) {
    return safeError(`Access denied: ${userRole} cannot perform ${actionName}`, 'access-denied-role');
  }
...
export const serverConfig: ServerConfig = {
  // ... other config
  onBeforeActionHandler: globalAccessControl,
};
```
...
**Input Parameters (single object):**
...
- `action`: Full Action object being executed
...
- `safeError(message, error_id, extra?)`: Deny action with custom error message and error id

**OnAfterActionHandler:**

**Input Parameters (single object):**
- `nileContext`: NileContext with user, session, request
- `action`: Full Action object that was executed
- `payload`: Request payload
- `result`: SafeResult from action execution

**Return Values:**
- `Ok(data, message?)`: Accept result (can transform if needed)
- `safeError(message, error_id, extra?)`: Override with error
...
**1. Role-Based Access Control (Before Hook):**

```typescript
import type { OnBeforeActionHandler } from '@nile-squad/nile/types';
import { Ok, safeError } from '@nile-squad/nile/utils/safe-try';

const roleBasedAccess: OnBeforeActionHandler = ({ nileContext, action, payload }) => {
  const userRole = nileContext.user?.role;
  const actionName = `${action.serviceName}.${action.name}`;
  
  // Define hierarchical permissions
  if (actionName.startsWith('admin.') && userRole !== 'admin') {
    return safeError('Administrative privileges required', 'admin-required');
  }
  
  if (action.name.includes('delete') && !['admin', 'moderator'].includes(userRole)) {
...
**2. Organization Data Isolation (Before Hook):**

```typescript
const organizationIsolation: OnBeforeActionHandler = ({ nileContext, action, payload }) => {
  const userOrgId = nileContext.user?.organization_id;
...
**3. Rate Limiting (Before Hook):**

```typescript
const rateLimiting: OnBeforeActionHandler = async ({ nileContext, action, payload }) => {
  const userId = nileContext.user?.id;
  const limits = { user: 100, admin: 1000 }; // requests per hour
  
  if (await isRateLimited(userId, limits[nileContext.user?.role])) {
...
### Global Action Hooks vs Action-Level Hooks

| Feature | Global Action Hooks | Action-Level Hooks |
|---------|----------------------|-------------------|
| **Scope** | All actions across all services | Specific action only |
| **Purpose** | Cross-cutting concerns (auth, rate limiting, auditing) | Business workflow logic |
| **Execution** | Before/after every action | Within action workflow |
| **Types** | `onBeforeActionHandler`, `onAfterActionHandler` | `hooks.before`, `hooks.after` |
| **Return Type** | `Ok(data, message?) \| safeError(message, error_id, extra?)` | `SafeResult<T>` |
| **Configuration** | Server config (`onBeforeActionHandler`, `onAfterActionHandler`) | Individual action (`hooks` property) |
| **Data Flow** | Approves/denies or audits execution | Transforms action data |
| **Parameters** | `{ nileContext, action, payload }` (before) or `{ nileContext, action, payload, result }` (after) | Varies by hook type |
...
```json
// Hook denial response (onBeforeActionHandler)
{
  "status": false,
...
2. **Fail Secure**: Deny access when in doubt (especially in before hooks)
...
6. **Separate Concerns**: Use `onBeforeActionHandler` for authorization, `onAfterActionHandler` for auditing
7. **Return SafeResult**: Always return `Ok()` or `safeError()`, never throw
