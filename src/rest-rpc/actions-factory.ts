...
import { safeError, Ok } from "../utils";
...
const generateGetAllAction = (
	...
	const getAllActionHandler: ActionHandler = async (data) => {
		...
		if (errors.length) {
			...
			return safeError(`safeError getting all records from ${tableName}`, error_id);
		}
		return Ok(result as any);
	};
...
const generateGetEveryAction = (
	...
	const getEveryActionHandler: ActionHandler = async (data) => {
		...
		if (errors.length) {
			...
			return safeError(`safeError getting every record from ${tableName}`, error_id);
		}
		return Ok(result as any);
	};
...
export const generateDeleteAllAction = (
	...
	const deleteAllActionHandler: ActionHandler = async (data) => {
		...
		if (errors.length) {
			...
			return safeError(`safeError deleting all records from ${tableName}`, error_id);
		}
		return Ok(result as any);
	};
