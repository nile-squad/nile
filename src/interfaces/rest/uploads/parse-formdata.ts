...
const DEFAULT_MIN_FILE_SIZE = 1; // 1 byte (no zero-byte files by default)
...
/**
 * Validate minimum file size limit
 * @param files - Array of File objects to validate
 * @param minFileSize - Minimum allowed size per file
 * @returns Validation result
 */
...
export function validateFiles(
  ...
  const minSizeCheck = validateMinFileSize(files, minFileSize);
  if (!minSizeCheck.status) {
    return minSizeCheck;
  }
